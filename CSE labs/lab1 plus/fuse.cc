...
getattr(yfs_client::inum inum, struct stat &st)
{
    ...
    st.st_ino = inum;
    int type = yfs->check_type(inum);
    // printf("getattr %016llx %d\n", inum, type == extent_protocol::T_FILE);
        ...
        int to_set, struct fuse_file_info *fi)
{
    ...
    if ((FUSE_SET_ATTR_CTIME | FUSE_SET_ATTR_MTIME | FUSE_SET_ATTR_ATIME | FUSE_SET_ATTR_SIZE) & to_set) {
...
void fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char*name){
    ...
    int r;
    yfs_client::inum ino;
    if((r = yfs->symlink(parent, link, name, ino)) == yfs_client::OK){
        e.ino = ino;
        getattr(ino, e.attr);
        fuse_reply_entry(req, &e);
    }
