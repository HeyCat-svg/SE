...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch(op){
        ...
        case A_SUB:val = argA - argB;break;
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = FALSE;
    if(op==A_ADD&&((argA>0&&argB>0&&sign)||(argA<0&&argB<0&&!sign)))
        ovf = TRUE;
    if(op==A_SUB&&((argA>0&&argB<0&&sign)||(argA<0&&argB>0&&!sign)))
        ovf = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch(cond){
        ...
        case C_LE:doit = (s^o)|z;break;
        case C_L:doit = s ^ o;break;
        case C_E:doit = z;break;
        case C_NE:doit = ~z;break;
        case C_GE:doit = ~(s^o);break;
        case C_G:doit = (~(s^o))&(~z);break;
    }
    doit = doit & 0x1;
    return doit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if (icode == I_RRMOVQ || icode == I_IRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ || icode == I_ALU || icode == I_PUSHQ || icode == I_POPQ){
        if (!get_byte_val(sim->m, next_pc, &rcode)) {
            err_print("PC = 0x%lx, Invalid register address", sim->pc);
            return STAT_INS;
        }
        regA = GET_REGA(rcode);
        regB = GET_REGB(rcode);
        if((regA<REG_RAX&&regA>REG_NONE)||(regB<REG_RAX&&regB>REG_NONE)){
            err_print("PC = 0x%lx, Invalid register id %.2x\t%.2x", sim->pc,regA,regB);
            return STAT_INS;
        }
        next_pc++;
    }
    ...
    if (icode == I_IRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ || icode == I_JMP || icode == I_CALL){
        if (!get_long_val(sim->m, next_pc, &value)) {
            err_print("PC = 0x%lx, Invalid immediate address", sim->pc);
            return STAT_INS;
        }
        next_pc += 8;
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:{      /* 2:x regA:regB */
            if(regA==REG_NONE||regB==REG_NONE){
                err_print("PC = 0x%lx, Invalid register id(irmovq) %.2x", sim->pc, regB);
                return STAT_INS;
            }
            switch(ifun){
              case 0:
                set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));break;
              case 1:
                if(cond_doit(sim->cc,C_LE))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              case 2:
                if(cond_doit(sim->cc,C_L))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              case 3:
                if(cond_doit(sim->cc,C_E))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              case 4:
                if(cond_doit(sim->cc,C_NE))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              case 5:
                if(cond_doit(sim->cc,C_GE))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              case 6:
                if(cond_doit(sim->cc,C_G))
                    set_reg_val(sim->r,regB,get_reg_val(sim->r,regA));
                break;
              default:
                err_print("PC = 0x%lx, Invalid instruction(rrmovq) %.2x", sim->pc, codefun);
    	        return STAT_INS;
            }
            sim->pc = next_pc;
            break;
      ...
      case I_IRMOVQ:{
            if(regB==REG_NONE){/* 3:0 F:regB imm */
                err_print("PC = 0x%lx, Invalid register id(irmovq) %.2x", sim->pc, regB);
                return STAT_INS;
            }
            set_reg_val(sim->r,regB,value);
            sim->pc = next_pc;
            break;
        }  
      case I_RMMOVQ:{/* 4:0 regA:regB imm */
            if(regA==REG_NONE||regB==REG_NONE){
                err_print("PC = 0x%lx, Invalid register id(rmmovq) %.2x", sim->pc, rcode);
                return STAT_INS;
            }
            if(!set_long_val(sim->m,get_reg_val(sim->r,regB)+value,get_reg_val(sim->r,regA))){
                err_print("PC = 0x%lx, Invalid memory address(rmmovq) 0x%lx", sim->pc, get_reg_val(sim->r, regB) + value);
                return STAT_INS;
            }
            sim->pc = next_pc;
            break;
      ...
      case I_MRMOVQ:{/* 5:0 regB:regA imm */
            if(regA==REG_NONE||regB==REG_NONE){
                err_print("PC = 0x%lx, Invalid register id(mrmovq) %.2x", sim->pc, rcode);
                return STAT_INS;
            }
            long_t tmp;
            if(!get_long_val(sim->m,get_reg_val(sim->r,regB)+value,&tmp)){
                err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB) + value);
                return STAT_ADR;
            }
      ...
      case I_ALU: {/* 6:x regA:regB */
            if(regA==REG_NONE||regB==REG_NONE){
                err_print("PC = 0x%lx, Invalid register id(OPq) %.2x", sim->pc, rcode);
                return STAT_INS;
            }
            long_t v1=get_reg_val(sim->r,regA);
            long_t v2=get_reg_val(sim->r,regB);
            switch(ifun){
              case 0: 
                set_reg_val(sim->r,regB,v2+v1);
                sim->cc = compute_cc(A_ADD, v2, v1, v1 + v2);
                break;
              case 1:
                set_reg_val(sim->r,regB,v2-v1);
                sim->cc = compute_cc(A_SUB, v2, v1, v2 - v1);
                break;
              case 2:
                set_reg_val(sim->r,regB,v2&v1);
                sim->cc = compute_cc(A_AND, v2, v1, v1 & v2);
                break;
              case 3:
                set_reg_val(sim->r,regB,v2^v1);
                sim->cc = compute_cc(A_XOR, v2, v1, v1 ^ v2);
                break;
              default:
                err_print("PC = 0x%lx, Invalid instruction(OPq) %.2x", sim->pc, codefun);
    	        return STAT_INS;
            }
            sim->pc = next_pc;
            break;
      ...
      case I_JMP:{/* 7:x imm */
            switch(ifun){
              ...
              case 6:
                ...
                break;
              default:
                err_print("PC = 0x%lx, Invalid instruction(jmp) %.2x", sim->pc, codefun);
    	        return STAT_INS;
            }
            break;
        }
      ...
      case I_RET:{/* 9:0 */
            ...
            if(!get_long_val(sim->m, rsp, &(sim->pc))){
                err_print("PC = 0x%lx, Invalid memory address(ret)", sim->pc);
                return STAT_INS;
            }
            rsp += 8;
            set_reg_val(sim->r, REG_RSP, rsp);
      ...
      case I_PUSHQ:{/* A:0 regA:F */
            if(regA==REG_NONE){
                err_print("PC = 0x%lx, Invalid register id(pushq) %.2x", sim->pc, rcode);
                return STAT_INS;
            }
            long_t rsp = get_reg_val(sim->r, REG_RSP);
            long_t tmp = get_reg_val(sim->r, regA);
            rsp -= 8;
            set_reg_val(sim->r, REG_RSP, rsp);
            if(!set_long_val(sim->m,rsp,tmp)){
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
                return STAT_ADR;
            }
            sim->pc = next_pc;
