...
parse_t parse_symbol(char **ptr, char **name){
    ...
    int i = 0;
    while(!IS_BLANK(*ptr+i)&&!IS_END(*ptr+i)){++i;}
...
parse_t parse_digit(char **ptr, long *value){
    ...
    char *endptr = NULL;
    long num = (long)strtol(*ptr, &endptr, 0);
    /* set 'ptr' and 'value' */
    *value = num;
    *ptr = endptr;
    return PARSE_DIGIT;
...
*           call SUM  #invoke SUM function */
    ...
    /* is a comment ? */
    if(IS_COMMENT(p->y64asm)){
        p->type = TYPE_COMM;
        return p->type;
    }
        /* is a label ? */
        char *name = NULL;
        if (parse_label(&(p->y64asm), &name)==PARSE_LABEL){
            ...
            SKIP_BLANK(p->y64asm);
            if(IS_END(p->y64asm)||IS_COMMENT(p->y64asm))
                return p->type;
        }
        /* is an instruction ? */
        instr_t *instruction = NULL;
        if (parse_instr(&(p->y64asm), &instruction)!=PARSE_INSTR){
            ...
            if(instrCode==I_NOP||instrCode==I_HALT||instrCode==I_RET){
                ...
                regid_t regA = 0xf;
                regid_t regB = 0xf;
                if(parse_reg(&(p->y64asm),&regA)!=PARSE_REG){
                    err_print("Invalid REG");
                    p->type = TYPE_ERR;
                    return p->type;
                }
                if(parse_delim(&(p->y64asm),',')!=PARSE_DELIM){
                    err_print("Invalid \',\'");
                    p->type = TYPE_ERR;
                    return p->type;
                }
                if(parse_reg(&(p->y64asm),&regB)!=PARSE_REG){
                    err_print("Invalid REG");
                    p->type = TYPE_ERR;
                    return p->type;
                }
                ...
                regid_t regA = 0xf;
                regid_t regB = 0xf;
                parse_t ret = parse_imm(&(p->y64asm), &name, &value);
                if(ret == PARSE_ERR){
                ...
                if(parse_mem(&(p->y64asm),&value,&regB)!=PARSE_MEM){
                ...
                *((long *)(&(p->y64bin.codes[2]))) = value;
                ...
                char *name = NULL;
                long value = 0;
                parse_t ret = parse_data(&(p->y64asm),&name,&value);
                if (ret==PARSE_DIGIT){
                    *((long *)(&(p->y64bin.codes[1]))) = value;
                    return p->type;
                }
                else if(ret==PARSE_SYMBOL){
                    add_reloc(name, &(p->y64bin));
                    ...
                    err_print("Line number:%2d Illigal data.(%s)", lineno, p->y64asm);
                    p->type = TYPE_ERR;
                    return p->type;
                }
            }
                ...
                switch(funCode){
                    ...
                    case D_POS:{
                        if(parse_digit(&(p->y64asm),&value)!=PARSE_DIGIT){
